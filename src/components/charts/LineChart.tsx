...
// Line chart component with D3.js animations and teal/cyan theme
...
export function LineChart({ data, color, maxValue, height = 240, className }: LineChartProps) {
  ...
  useEffect(() => {
    ...
    const margin = { top: 32, right: 32, bottom: 36, left: 56 }
    ...
    // Draw grid lines with teal tint
    ...
    yTicks.forEach(tick => {
      g.append('line')
        ...
        .attr('y1', yScale(tick))
        .attr('y2', yScale(tick))
        .attr('stroke', 'rgba(6, 182, 212, 0.13)')
        .attr('stroke-width', 1)
    ...
    // Draw data points with glow and outer ring
    data.forEach((d, i) => {
